...
module.exports = (grunt) ->
  ...
  grunt.initConfig
    ...
    coffee:
      compile:
        files:
          'backbone.viewmodel.js': [ 'src/backbone.viewmodel.coffee' ]
    ...
    uglify:
      main:
        files:
          'backbone.viewmodel.min.js': [ 'backbone.viewmodel.js' ]

    karma:
      options:
        browsers: [ 'PhantomJS' ]
        frameworks: [ 'mocha', 'chai' ]
        files: [ 'backbone.viewmodel.js', 'test/**/*.coffee' ]
    ...
    replace:
      configVersion:
        src: [ 'src/backbone.viewmodel.coffee' ]
        overwrite: true
        replacements: [
          from: /VERSION: '.*?'/i
      ...
      minJS:
        src: [ 'backbone.viewmodel.min.js' ]
        overwrite: true
        replacements: [
          from: /^/
          to: -> "/* backbone.viewmodel.min.js v#{ packageVersion() } */ \n"
        ]
    ...
    bump:
      options:
        ...
        commitFiles: [
          'package.json'
          'src/backbone.viewmodel.coffee'
          'backbone.viewmodel.js'
          'backbone.viewmodel.min.js'
        ]
        pushTo: 'origin'
